...
import { DshThemeFeedback, DshThemeMarketPanel, ThemeDetailsDialog, ThemePreview, runtimeFor, type DshThemeOperationNotice } from "./DshThemeMarketPanel";
...
export function DshThemesPage({ active, snapshot: sharedSnapshot, snapshotGeneration, onSnapshotChange: sharedSnapshotChange, operationKey: sharedOperationKey, operationProgress: sharedOperationProgress, notice: sharedNotice, noticeFading: sharedNoticeFading, onBusyChange: sharedBusyChange, onProgressChange: sharedProgressChange, onNoticeChange: sharedNoticeChange }: DshThemesPageProps) {
  ...
  const [localOperationProgress, setLocalOperationProgress] = useState<DshSkinOperationProgress | null>(null);
  const [localNotice, setLocalNotice] = useState<DshThemeOperationNotice | null>(null);
  const [localNoticeFading, setLocalNoticeFading] = useState(false);
  ...
  const onBusyChange = sharedBusyChange ?? setLocalOperationKey;
  const onProgressChange = sharedProgressChange ?? setLocalOperationProgress;
  ...
  useEffect(() => {
    if (sharedNoticeChange || !localNotice || localNotice.persistent) {
      if (!localNotice || localNotice.persistent) setLocalNoticeFading(false);
      return undefined;
    }
    setLocalNoticeFading(false);
    const fadeTimer = window.setTimeout(() => setLocalNoticeFading(true), 9_700);
    const clearTimer = window.setTimeout(() => setLocalNotice(null), 10_000);
    return () => {
      window.clearTimeout(fadeTimer);
      window.clearTimeout(clearTimer);
    };
  }, [localNotice, sharedNoticeChange]);
  ...
  const operationLocked = operationKey !== null || operationProgress !== null;
  ...
  async function mutate(skin: Pick<DshSkinCatalogEntry, "id">, action: DshSkinAction) {
    ...
    if (busyRef.current !== null || operationLocked) {
      ...
      <section className="dsh-theme-library-content" aria-live="polite" tabIndex={0} aria-label={t("dshThemes.libraryContent", "已安装主题")}>
        ...
        {!loading && snapshot && installed.length === 0 && localInstalled.length === 0 ? (
          <div className="dsh-theme-empty">
            ...
            <button type="button" className="dsh-theme-primary-button" onClick={() => setMarketOpen(true)} disabled={operationLocked}><Store size={15} />{t("dshThemes.browseMarket", "浏览主题市场")}</button>
          </div>
        ) : null}
