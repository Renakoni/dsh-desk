...
import { ArrowLeft, Check, ChevronLeft, ChevronRight, Clock3, Download, ExternalLink, ImageOff, Info, Power, RefreshCw, Search, Star, Trash2, X } from "lucide-react";
  ...
  const label = progress.phase === "queued"
    ...
    : progress.phase === "downloading"
      ...
      : progress.phase === "checking"
        ? t("dshThemes.progressChecking", "检查主题兼容性…")
...
export function DshThemeMarketPanel({ initialSnapshot, onBack, onChanged }: {
  ...
  return (
    <div className="settings-page dsh-themes-page dsh-theme-market">
      ...
      <section className="dsh-theme-market-scroll" aria-live="polite" tabIndex={0} aria-label={t("dshThemes.marketContent", "主题市场列表")}>
        ...
        {snapshot && rows.length > 0 ? <div className="dsh-theme-market-grid">{rows.slice(0, visibleCount).map(skin => {
          ...
          return (
            <article key={skin.id} className={`dsh-theme-market-card ${activeTheme ? "active" : ""}`} data-testid="dsh-theme-card">
              ...
              <div className="dsh-theme-market-card-footer"><button type="button" className="text" onClick={event => openDetails(skin, event.currentTarget)}>{t("dshThemes.details", "详情")}</button>{installed ? activeTheme ? <button type="button" disabled>{t("dshThemes.inUse", "使用中")}</button> : <button type="button" className="primary" disabled={!canManage || isBusy} aria-describedby={!canManage ? "dsh-theme-market-host-status" : undefined} onClick={() => void mutate(skin, "activate")}>{isBusy ? t("dshThemes.working", "处理中…") : t("dshThemes.use", "使用")}</button> : <button type="button" className="primary" disabled={!canManage || isBusy} aria-describedby={!canManage ? "dsh-theme-market-host-status" : undefined} onClick={() => void mutate(skin, "install")} title={!canManage ? t("dshThemes.startDshToManage", "启动 DSH 后可安装主题") : undefined}>{isBusy ? t("dshThemes.working", "处理中…") : manual ? t("dshThemes.repository", "查看仓库") : t("dshThemes.install", "安装")}</button>}</div>
            </article>
          );
        })}</div> : null}
...
export function ThemeDetailsDialog({ skin, runtime, snapshot, shotIndex, busy, locale, t, onShotIndex, onClose, onMutate }: {
  ...
  const canManage = snapshot.host.connected;
  const hostStatusId = canManage ? undefined : "dsh-theme-market-host-status";
  const manual = skin.review?.installation === "manual-only";
  ...
  return <div className="dsh-theme-dialog-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section ref={dialogRef} tabIndex={-1} className="dsh-theme-dialog" role="dialog" aria-modal="true" aria-label={locale === "zh" ? skin.name.zh : skin.name.en}>
    ...
    <div className="dsh-theme-dialog-copy"><div className="dsh-theme-dialog-title"><div><span>{skin.author}</span><h3>{locale === "zh" ? skin.name.zh : skin.name.en}</h3></div><span className="dsh-theme-stars"><Star size={13} fill="currentColor" />{skin.stars === null ? "-" : skin.stars.toLocaleString()}</span></div><p>{skin.description}</p><div className="dsh-theme-tags">{skin.tags.slice(0, 8).map(tag => <span key={tag}>{tag}</span>)}</div><dl className="dsh-theme-metadata"><div><dt>{t("dshThemes.status", "状态")}</dt><dd><span>{status}</span>{updateAvailable ? <><span aria-hidden="true"> / </span><span>{updateLabel}</span></> : null}</dd></div><div><dt>{t("dshThemes.version", "版本")}</dt><dd title={t("dshThemes.versionDetail", "已安装版本 / 目录版本")}><span>{runtime?.installedVersion ?? "-"}</span><span aria-hidden="true"> / </span><span>{skin.install.version}</span></dd></div><div><dt>{t("dshThemes.catalogCommit", "目录 commit")}</dt><dd title={skin.install.commit}>{skin.install.commit ? shortCommit(skin.install.commit) : "-"}</dd></div><div><dt>{t("dshThemes.compatibility", "兼容性")}</dt><dd>{skin.compatibility.dsh} / {skin.modes.join(" / ")}</dd></div></dl>
      ...
      <div className="dsh-theme-dialog-actions">{skin.repositoryUrl ? <button type="button" className="dsh-theme-icon-button" onClick={() => void window.companion.openExternal(skin.repositoryUrl!)} title={t("dshThemes.openRepository", "打开仓库")} aria-label={t("dshThemes.openRepository", "打开仓库")}><ExternalLink size={17} /></button> : null}{installed ? <button type="button" className="danger-quiet" disabled={!canManage || busy} aria-describedby={hostStatusId} onClick={() => void onMutate(skin, "uninstall")}><Trash2 size={15} />{t("dshThemes.uninstall", "卸载")}</button> : null}{active ? <button type="button" disabled={!canManage || busy} aria-describedby={hostStatusId} onClick={() => void onMutate(skin, "deactivate")}><Power size={15} />{t("dshThemes.deactivate", "停用")}</button> : restartRequired ? <button type="button" className="primary" disabled={busy} onClick={() => void onMutate(skin, "restart")}><RefreshCw size={15} />{t("dshThemes.restartWeb", "重启 DSH")}</button> : installed ? <button type="button" className="primary" disabled={!canManage || busy} aria-describedby={hostStatusId} onClick={() => void onMutate(skin, "activate")}><Power size={15} />{t("dshThemes.use", "使用")}</button> : <button type="button" className="primary" disabled={!canManage || busy} aria-describedby={hostStatusId} onClick={() => void onMutate(skin, "install")}><Download size={15} />{manual ? t("dshThemes.repository", "查看仓库") : t("dshThemes.install", "安装")}</button>}{installed && updateAvailable ? <button type="button" className="primary" disabled={!canManage} aria-describedby={hostStatusId} onClick={() => void onMutate(skin, "update")}>{t("dshThemes.update", "更新")}</button> : null}</div>
    </div>
  </section></div>;
}
