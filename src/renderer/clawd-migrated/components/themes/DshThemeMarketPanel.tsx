...
export function ThemePreview({ skin, eager = false }: { skin: DshSkinCatalogEntry; eager?: boolean }) {
  ...
  if (!source || failed) return <div className="dsh-theme-preview-fallback" role="img" aria-label={`${skin.name.zh} 暂无预览`}><ImageOff size={24} /><span>{skin.author}</span></div>;
  return <img src={source} alt={`${skin.name.zh} 界面预览`} loading={eager ? "eager" : "lazy"} decoding="async" onError={() => setFailed(true)} />;
}
    ...
    <div className="settings-page dsh-themes-page dsh-theme-market">
      <header className="dsh-theme-market-header">
        ...
        <div className="dsh-theme-header-actions">
          <button type="button" className="dsh-theme-icon-button" onClick={() => void window.companion.openExternal("https://kingofsoysauce.github.io/dsh-skin-market/")} title={t("dshThemes.openOnlineMarket", "打开在线市场")} aria-label={t("dshThemes.openOnlineMarket", "打开在线市场")}><ExternalLink size={17} /></button>
      ...
      {selected && snapshot ? <ThemeDetailsDialog skin={selected} runtime={selectedRuntime} snapshot={snapshot} shotIndex={shotIndex} busy={busy !== null} locale={locale} t={t} onShotIndex={setShotIndex} onClose={() => setSelectedId(null)} onMutate={mutate} /> : null}
    </div>
  );
}

function ThemeDetailsDialog({ skin, runtime, snapshot, shotIndex, busy, locale, t, onShotIndex, onClose, onMutate }: {
  ...
  const manual = skin.review?.installation === "manual-only";
  return <div className="dsh-theme-dialog-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="dsh-theme-dialog" role="dialog" aria-modal="true" aria-label={locale === "zh" ? skin.name.zh : skin.name.en}>
    <button type="button" className="dsh-theme-dialog-close" onClick={onClose} aria-label={t("dshThemes.closeDetails", "关闭详情")}><X size={18} /></button>
    <div className="dsh-theme-dialog-media"><ThemePreview key={`${skin.id}:${shotIndex}`} skin={{ ...skin, listScreenshot: skin.screenshots[shotIndex] ?? skin.listScreenshot }} eager />{skin.screenshots.length > 1 ? <><button type="button" className="previous" onClick={() => onShotIndex((shotIndex - 1 + skin.screenshots.length) % skin.screenshots.length)} aria-label={t("dshThemes.previousImage", "上一张预览")}><ChevronLeft size={20} /></button><button type="button" className="next" onClick={() => onShotIndex((shotIndex + 1) % skin.screenshots.length)} aria-label={t("dshThemes.nextImage", "下一张预览")}><ChevronRight size={20} /></button><span className="dsh-theme-image-count">{shotIndex + 1} / {skin.screenshots.length}</span></> : null}</div>
    <div className="dsh-theme-dialog-copy"><div className="dsh-theme-dialog-title"><div><span>{skin.author}</span><h3>{locale === "zh" ? skin.name.zh : skin.name.en}</h3></div><span className="dsh-theme-stars"><Star size={13} fill="currentColor" />{skin.stars.toLocaleString()}</span></div><p>{skin.description}</p><div className="dsh-theme-tags">{skin.tags.slice(0, 8).map(tag => <span key={tag}>{tag}</span>)}</div><dl className="dsh-theme-metadata"><div><dt>{t("dshThemes.version", "版本")}</dt><dd>{skin.install.version}</dd></div><div><dt>{t("dshThemes.updated", "更新")}</dt><dd>{formatDate(skin.updatedAt, locale)}</dd></div><div><dt>{t("dshThemes.mode", "模式")}</dt><dd>{skin.modes.join(" / ")}</dd></div><div><dt>{t("dshThemes.compatibility", "DSH")}</dt><dd>{skin.compatibility.dsh}</dd></div></dl>
      ...
      <div className="dsh-theme-dialog-actions"><button type="button" className="dsh-theme-icon-button" onClick={() => void window.companion.openExternal(skin.repositoryUrl)} title={t("dshThemes.openRepository", "打开仓库")} aria-label={t("dshThemes.openRepository", "打开仓库")}><ExternalLink size={17} /></button>{runtime?.installation === "installed" ? <button type="button" className="danger-quiet" disabled={!canManage || busy} onClick={() => void onMutate(skin, "uninstall")}><Trash2 size={15} />{t("dshThemes.uninstall", "卸载")}</button> : null}{runtime?.activation === "active" ? <button type="button" disabled={!canManage || busy} onClick={() => void onMutate(skin, "deactivate")}><Power size={15} />{t("dshThemes.deactivate", "停用")}</button> : runtime?.activation === "restart-required" && snapshot.host.restartAvailable ? <button type="button" className="primary" disabled={busy} onClick={() => void onMutate(skin, "restart")}><RefreshCw size={15} />{t("dshThemes.restartWeb", "重启 DSH")}</button> : runtime?.installation === "installed" ? <button type="button" className="primary" disabled={!canManage || busy} onClick={() => void onMutate(skin, runtime.updateAvailable ? "update" : "activate")}>{runtime.updateAvailable ? t("dshThemes.update", "更新") : t("dshThemes.use", "使用")}</button> : <button type="button" className="primary" disabled={!canManage || busy} onClick={() => void onMutate(skin, "install")}><Download size={15} />{manual ? t("dshThemes.repository", "查看仓库") : t("dshThemes.install", "安装")}</button>}</div>
    </div>
  </section></div>;
}
