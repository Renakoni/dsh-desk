...
import { DshSkinMarketplace, DSH_SKIN_CATALOG_URL, parseDshSkinCatalog, readManagedDshThemePackages, resolveDshSkinPreviewPath } from "../src/main/dshSkinMarketplace";

function response(body: unknown, status = 200, contentType?: string) {
  const text = JSON.stringify(body);
  const bytes = Buffer.from(text);
  return {
    ok: status >= 200 && status < 300,
    status,
    headers: { get: (name: string) => name === "content-length" ? String(bytes.byteLength) : contentType ?? null },
    text: async () => text,
...
function imageResponse(bytes: Uint8Array, contentType: string) {
  return {
    ok: true,
    status: 200,
    headers: { get: (name: string) => name === "content-length" ? String(bytes.byteLength) : name === "content-type" ? contentType : null },
    text: async () => "",
    arrayBuffer: async () => bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength) as ArrayBuffer
  };
}
...
describe("DshSkinMarketplace", () => {
  it("resolves only existing hashed preview assets inside the cache directory", () => {
    const root = mkdtempSync(join(tmpdir(), "dsh-skin-previews-"));
    const source = "https://example.com/theme.png";
    const digest = createHash("sha256").update(source).digest("hex");
    const filePath = join(root, `${digest}.png`);
    writeFileSync(filePath, Buffer.from([1, 2, 3]));

    expect(resolveDshSkinPreviewPath(`dsh-theme-asset://previews/${digest}.png`, root)).toBe(filePath);
    expect(resolveDshSkinPreviewPath(`dsh-theme-asset://previews/${digest}.png?x=1`, root)).toBeNull();
    expect(resolveDshSkinPreviewPath("dsh-theme-asset://other/preview.png", root)).toBeNull();
    expect(resolveDshSkinPreviewPath(`dsh-theme-asset://previews/${"a".repeat(64)}.png`, root)).toBeNull();
  });

  it("caches an installed theme thumbnail and all detail screenshots", async () => {
    const root = mkdtempSync(join(tmpdir(), "dsh-skin-preview-cache-"));
    const previewBytes = new Uint8Array([137, 80, 78, 71]);
    const thumbnailSource = "https://example.com/thumbnail.webp";
    const previewSource = "https://example.com/theme.png";
    const secondPreviewSource = "https://example.com/theme-dark.png";
    const catalog = skinCatalog() as any;
    catalog.skins[0].listScreenshot = thumbnailSource;
    catalog.skins[0].screenshots = [previewSource, secondPreviewSource];
    const thumbnailDigest = createHash("sha256").update(thumbnailSource).digest("hex");
    const previewDigest = createHash("sha256").update(previewSource).digest("hex");
    const secondPreviewDigest = createHash("sha256").update(secondPreviewSource).digest("hex");
    const fetcher = vi.fn(async (url: string) => {
      if (url === DSH_SKIN_CATALOG_URL) return response(catalog);
      if (url === thumbnailSource) return imageResponse(previewBytes, "image/webp");
      if (url === previewSource || url === secondPreviewSource) return imageResponse(previewBytes, "image/png");
      return response({ skins: [{ skinId: "demo.skin", installation: "installed", activation: "inactive", installedVersion: "1.0.0", installedAt: null, updateAvailable: false }], restartAvailable: false });
    });
    const market = new DshSkinMarketplace({
      ...
      fetcher,
      previewCacheDir: join(root, "previews"),
      previewAssetBaseUrl: "dsh-theme-asset://previews"
    });

    const first = await market.snapshot();
    expect(first.skins[0].previewLocalUrl).toBe(`dsh-theme-asset://previews/${thumbnailDigest}.webp`);
    expect(first.skins[0].previewLocalUrls).toEqual([
      `dsh-theme-asset://previews/${previewDigest}.png`,
      `dsh-theme-asset://previews/${secondPreviewDigest}.png`
    ]);
    expect(readFileSync(join(root, "previews", `${thumbnailDigest}.webp`))).toEqual(Buffer.from(previewBytes));
    expect(readFileSync(join(root, "previews", `${previewDigest}.png`))).toEqual(Buffer.from(previewBytes));
    expect(readFileSync(join(root, "previews", `${secondPreviewDigest}.png`))).toEqual(Buffer.from(previewBytes));
    expect(fetcher.mock.calls.filter(([url]) => url === previewSource)).toHaveLength(1);
    expect(fetcher.mock.calls.filter(([url]) => url === secondPreviewSource)).toHaveLength(1);

    const second = await market.snapshot();
    expect(second.skins[0].previewLocalUrl).toBe(first.skins[0].previewLocalUrl);
    expect(second.skins[0].previewLocalUrls).toEqual(first.skins[0].previewLocalUrls);
    expect(fetcher.mock.calls.filter(([url]) => url === previewSource)).toHaveLength(1);
    expect(fetcher.mock.calls.filter(([url]) => url === secondPreviewSource)).toHaveLength(1);
  });
