...
describe("DshSkinMarketplace", () => {
  ...
  it("caches an installed theme thumbnail and reuses it on later snapshots", async () => {
    ...
    const digest = createHash("sha256").update(previewSource).digest("hex");
    const fetcher = vi.fn(async (url: string) => {
      if (url === DSH_SKIN_CATALOG_URL) return response(skinCatalog());
      if (url === previewSource) return imageResponse(previewBytes, "image/png");
    ...
    expect(first.skins[0].previewLocalUrl).toBe(`dsh-theme-asset://previews/${digest}.png`);
    expect(readFileSync(join(root, "previews", `${digest}.png`))).toEqual(Buffer.from(previewBytes));
